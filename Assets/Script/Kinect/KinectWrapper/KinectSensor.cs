...
public class KinectSensor : MonoBehaviour, KinectInterface {
    ...
    public bool fastDepth = true;
    public NuiImageResolution DepthResolution {
        get { return fastDepth ? NuiImageResolution.resolution320x240 : NuiImageResolution.resolution640x480; }
    }
	...
	void Awake()
	{
		...
		try
		{
			...
			hr = NativeMethods.NuiImageStreamOpen(NuiImageType.DepthAndPlayerIndex, DepthResolution, 0, 2, IntPtr.Zero, ref depthStreamHandle);
	...
	bool KinectInterface.pollDepth()
	{
		if (!updatedDepth)
		{
			...
			int hr = NativeMethods.NuiImageStreamGetNextFrame(depthStreamHandle, 1, ref imageFramePtr);
			if (hr == 0){
				newDepth = true;
    ...
    public void DoWithLockedDepth(Func<IntPtr> f) {
